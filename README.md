...
1. **克隆项目**
   ```bash
   git clone https://github.com/Shirolin/cf-speedtest-sync.git
...
项目会自动将 IP 同步记录写入到 `output/<你的域名>/sync.log`。测速模块与主控脚本共用同一个日志文件，因此优选 API 超时、`cfst` 无结果、锁冲突等失败原因也都能在这里看到。判断某天是否真的改写过 DNS，看当日是否同时存在 `>>> Syncing IPv4` 与 `>>> Sync completed.` 两行。为了方便查看历史轨迹，项目提供了查询过去一周（7天内）IP 同步日志的脚本：
